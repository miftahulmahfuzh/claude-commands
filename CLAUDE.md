...
- `/implement -f <SLUG>_PLAN.md` → **executes**. Creates one task per phase, copies each phase plan to `{pkg}/.workflows/plan/{TaskID}.md` unchanged, applies one phase, hands to `completion-handler`. It writes no plans; handed an analysis document it refuses and names `/analyze`. When the phase it just finished was the plan set's last (a single-phase plan, or `--all` reaching the end) and no swarm coordinator owns it, **it lands the branch itself** — Step 5a merges into `main` in a throwaway worktree, pushes, then deletes the plan's worktree and branch — rather than printing a `git merge` command for a human to run. That merge-it-yourself branch is exactly what closes the gap `/analyze`'s "nothing to land except one commit" (above) glosses over: one commit still has to reach `main`, and for N=1 there is no orchestrator coming to do it.
