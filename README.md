...
| Development | the skill runs the loop, in a **worktree off `origin/main`**, ending in a **PR it merges itself** | mints a TaskID and hands off to **`/do`** |
| `Completed` | the linked PR is **merged** | the issue is **closed** |
| Who merges | **the session**, gated on the repo's own CI | `/do`, on its own branch rules |
